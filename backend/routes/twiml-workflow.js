...
<Response>
    ...
    if (!currentNodeId) {
      ...
      if (startNode) {
        ...
        // Use first node if no start node found
        currentNodeId = workflowData.nodes[0]?.id;
        console.log('Using first node as start:', currentNodeId);
      }
    }
...
<Response>
    ...
    // Generate TwiML based on the current node
    let twiml;

    if (!speechResult && !digits) {
      // First interaction or no user input - use node's prompt
...
<Response>
    ...
    <Hangup/>
      ...
      // User provided input - find next node or respond
      ...
      if (outgoingEdges.length > 0) {
        // Use first outgoing edge for now (can be enhanced with conditions later)
